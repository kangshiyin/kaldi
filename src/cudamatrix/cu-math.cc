...
void CpuBackpropLstmNonlinearity(const MatrixBase<Real> &input,
  ...
  const MatrixBase<BaseFloat> &input_mat = input;
  const MatrixBase<BaseFloat> &params_mat = params;
  const MatrixBase<BaseFloat> &output_deriv_mat = output_deriv;
  const MatrixBase<double> &deriv_sum_in_mat = deriv_sum_in;
  const VectorBase<BaseFloat> &sr_config = self_repair_config;
  MatrixBase<BaseFloat> *input_deriv_mat = (
      input_deriv == NULL ? NULL : input_deriv);
  MatrixBase<BaseFloat> *params_deriv_mat = NULL;
  MatrixBase<BaseFloat> *self_repair_sum_out_mat = NULL;
  ...
  for (int32 c = 0; c < cell_dim; c++) {
    ...
    BaseFloat w_fc = params_mat(1, c);
    BaseFloat w_oc = params_mat(2, c);
    // derivative sums w.r.t. parameters.
    BaseFloat w_ic_deriv_sum = 0.0;
    BaseFloat w_fc_deriv_sum = 0.0;
    BaseFloat w_oc_deriv_sum = 0.0;
    ...
    BaseFloat f_t_value_sum = 0.0, f_t_deriv_sum = 0.0;
    BaseFloat c_part_value_sum = 0.0, c_part_deriv_sum = 0.0;
    BaseFloat o_t_value_sum = 0.0, o_t_deriv_sum = 0.0;
    BaseFloat c_t_value_sum = 0.0, c_t_deriv_sum = 0.0;
    ...
    for (int32 r = 0; r < num_rows; r++) {
      ...
      BaseFloat dm_t = output_deriv_mat(r, c + cell_dim);
      BaseFloat dtanh_c_t = o_t * dm_t;
      BaseFloat do_t = tanh_c_t * dm_t;
      BaseFloat do_t_input = (o_t * (1.0F - o_t) * do_t - (2.0F * o_t - 1.0F) * o_t_self_repair);
      BaseFloat dc_t = ((1.0F - tanh_c_t * tanh_c_t) * dtanh_c_t + dc_t_out + do_t_input * w_oc) - tanh_c_t * c_t_self_repair;
      BaseFloat dtanh_c_part = i_t * dc_t;
      BaseFloat df_t = dc_t * c_prev;
      BaseFloat df_t_input = (df_t * f_t * (1.0F - f_t) - (2.0F * f_t - 1.0F) * f_t_self_repair);
      BaseFloat di_t = dc_t * tanh_c_part;
      BaseFloat di_t_input = (di_t * i_t * (1.0F - i_t) - (2.0F * i_t - 1.0F) * i_t_self_repair);
      ...
      BaseFloat do_part = do_t_input;
      BaseFloat dc_part = ((1.0F - tanh_c_part * tanh_c_part) * dtanh_c_part
          - tanh_c_part * c_part_self_repair);
      BaseFloat df_part = df_t_input;
      BaseFloat di_part = di_t_input;
